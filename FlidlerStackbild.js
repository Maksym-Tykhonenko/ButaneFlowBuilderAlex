...
import FolstornnerScreen from './ButaneFlowBuilderSrc/ButaneFlowBuilderScreens/ButalobuilGeneralAppcontainer';
import { createNativeStackNavigator as buildButaneStack } from '@react-navigation/native-stack';
import LoadingStormView from './ButaneFlowBuilderSrc/ButaneFlowBuilderScreens/ButaneFlowBuilderLoading';
import { SafeAreaProvider as SafeShellWrap } from 'react-native-safe-area-context';
import { GestureHandlerRootView as GestRootShell } from 'react-native-gesture-handler';
import OnboardTubeView from './ButaneFlowBuilderSrc/ButaneFlowBuilderScreens/ButaneFlowBuilderOnboarding';
import { NavigationContainer as NavShellPrime } from '@react-navigation/native';
import ProductScreen from './ButaneFlowBuilderSrc/ButaneFlowBuilderScreens/ProductScreen';

const TubenStacker = buildButaneStack();
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const ButaneRoutesChaos = () => {
  const NavigatorShell = TubenStacker.Navigator;
  const ScreenFluxer = TubenStacker.Screen;

  const chaosTransitions = {
    driftFade: { headerShown: false, animation: 'fade_from_bottom' },
    softGlim: { headerShown: false, animation: 'fade' },
  };
  //////////////////////////////////////////////////////////////////////////////////////////
  ...
  const INITIAL_URL = `https://bold-node-node.site/`;
  const URL_IDENTIFAIRE = `T6z3NCO0`;
  ...
  OneSignal.initialize('85cf9f70-3988-4638-85ef-a5250edc317e');
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <NavigatorShell screenOptions={{ headerShown: false }}>
          <ScreenFluxer
            initialParams={{
              responseToPushPermition,
              product: finalLink,
    ...
    return (
      <NavigatorShell
        ...
        initialRouteName="ButaneFlowBuilderOnboarding"
      >
        <ScreenFluxer
          name="ButaneFlowBuilderLoading"
          component={LoadingStormView}
          options={{
            ...chaosTransitions.driftFade,
            title: 'Storm Loading',
          }}
        />
        <ScreenFluxer
          name="ButalobuilGeneralAppcontainer"
          component={FolstornnerScreen}
          options={{
            ...chaosTransitions.driftFade,
        ...
        />
        <ScreenFluxer
          name="ButaneFlowBuilderOnboarding"
          component={OnboardTubeView}
          options={{
            ...chaosTransitions.softGlim,
            title: 'Onboard Tubes',
          }}
        />
      </NavigatorShell>
  ...
  return (
    <NavShellPrime>
      {!isLoading ? <LoadingStormView /> : <Route isFatch={route} />}
    </NavShellPrime>
  );
};

const TubularRootShell = () => {
  return (
    <GestRootShell style={{ flex: 1, backgroundColor: '#0D0D0E' }}>
      <SafeShellWrap>
        <ButaneRoutesChaos />
      </SafeShellWrap>
    </GestRootShell>
...
export default TubularRootShell;
